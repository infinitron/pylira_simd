...
def _describe_payload(self):
    ...
    print(
        tabulate([
            ...
            ], ["PSF shape", f"{self.psf.shape[0]}x{self.psf.shape[1]}"],
            [
                "Exposure map shape",
            ...
            ], ["Initial smoothing params (alpha_init)", self.alpha_init],
            ["Max iterations (max_iter)", self.max_iter],
            ["Output every (thin)", f"{self.thin} iterations"],
            ["Burn in iterations (burn_in)", self.burn_in],
            ["Output images path (out_img_file)", self.out_img_file],
            ["Output param file path (out_param_file)", self.out_param_file],
            ["Fit background scale (fit_bkgscl)", self.fit_bkgscl],
            [
            ...
            [
                ...
                self.ms_ttlcnt_exp
            ], [r'\kappa_1 (ms_al_kap1)', self.ms_al_kap1],
            [r'\kappa_2 (ms_al_kap2)', self.ms_al_kap2],
            [r'\kappa_3 (ms_al_kap3)', self.ms_al_kap3],
            [
